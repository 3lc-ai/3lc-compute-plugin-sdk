...
- :class:`HubPlugin` — root base (UI fragment + optional compute/routes/shutdown).
...
# both the Python surface (``HubPlugin`` / ``ComputePlugin`` / ``InfrastructurePlugin``
# / ``JobContext`` / ``shared.*``) and the browser surface (``PLUGIN_API`` / ``PluginJobs``
# / ``TlcData``, declared in ``contract/plugin-api.d.ts``) — and the thing a plugin pins
# (``>=X,<Y``). The host and frontend implement a range and compare compatibility on
# ``MAJOR.MINOR``.
try:
